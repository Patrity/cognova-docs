...
| Docker | 24+ | Local PostgreSQL (optional, or use a remote DB) |
...
| `pnpm release:next` | Publish pre-release to `@next` npm tag |
| `pnpm release:patch` | Bump patch, publish `@latest`, create GitHub release |
| `pnpm release:minor` | Bump minor, publish `@latest`, create GitHub release |
| `pnpm release:major` | Bump major, publish `@latest`, create GitHub release |
...
Work happens on `develop` (or feature branches off it). When ready to release:

```bash
# Pre-release: test on a real server without touching @latest
pnpm release:next
# → on test server: cognova update --channel next
