...
- **Local PostgreSQL** -- spins up a `postgres:16-alpine` Docker container on port 5432. Requires Docker (only dependency that uses Docker).
...
3. Runs `pnpm install`
4. Installs a system service (launchd on macOS, systemd on Linux)
5. Starts the app
