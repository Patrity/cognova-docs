...
```bash
cognova update [--channel <name>]
```

What it does:

1. Checks npm for the latest version on the active channel
...
By default, `cognova update` tracks the `@latest` channel (stable releases). You can opt into pre-release builds using the `--channel` flag:

```bash
# Switch to pre-release channel
cognova update --channel next

# Switch back to stable
cognova update
```

The active channel is stored in `.cognova` metadata and shown at the start of each update run.
...
The metadata file is written by `cognova init` and updated by `cognova update`. It contains the install path, vault path, installed version, active channel, and timestamps.
