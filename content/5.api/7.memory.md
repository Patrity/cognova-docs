...
The memory API manages knowledge chunks extracted from Claude Code sessions. Each chunk has a type, relevance score, and optional project association. For conceptual details, see [Claude Memory System](/claude/memory-system).
...
Memory types are defined in `shared/types/index.ts`. See [Database Schema](/architecture/database) for the full `memory_chunks` table definition.
